	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				// 你要清理数据
				// "phone_code:%s:%s"
				val, err := rdb.GetDel(ctx, "phone_code:login:15111112222").Result()
				cancel()
...
`,
		...
		},
		{
			...
			before: func(t *testing.T) {
				// 这个手机号码，已经有一个验证码了
				ctx, cancel := context.WithTimeout(context.Background(), time.Second*3)
				_, err := rdb.Set(ctx, "phone_code:login:15111112222", "123456",
...
`,
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				ctx, cancel := context.WithTimeout(context.Background(), time.Second*3)
				_, err := rdb.Set(ctx, "phone_code:login:15111112222", "123456", 0).Result()
				cancel()
				assert.NoError(t, err)
